...
public class DiceDodecahedronItem extends Item {

    public DiceDodecahedronItem(Settings settings) {
        super(settings);
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if(Screen.hasShiftDown()){
            tooltip.add(Text.translatable("roll.twelvetooltip").formatted(Formatting.DARK_PURPLE));
   ...
   private void outputRandomNumber(PlayerEntity player) {
    ...
    private int getRandomNumber() {
        return Random.createLocal().nextBetween(1,12);
}
